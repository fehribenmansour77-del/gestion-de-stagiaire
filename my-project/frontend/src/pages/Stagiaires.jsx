...
const Stagiaires = () => {
    ...
    const fetchInitialData = async () => {
      try {
        const [deptData, userData] = await Promise.all([
          ...
          userService.listUsers({ role: 'tuteur' })
        ]);
        setDepartements(deptData || []);
        setTuteurs(userData.data || userData || []);
  ...
  const handleExport = () => {
    ...
    const headers = ['Nom', 'Prénom', 'Email', 'Établissement', 'Filière', 'Statut', 'Date Démarrage', 'Date Fin'];
    const rows = stagiaires.map(s => [
      ...
      s.filiere || '',
      s.statut || '',
      s.date_demarrage ? new Date(s.date_demarrage).toLocaleDateString('fr-FR') : '',
    ...
    link.setAttribute("download", `Export_Stagiaires_GIAS_${new Date().toLocaleDateString('fr-FR').replace(/\//g, '-')}.csv`);
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 60, flexWrap: 'wrap', gap: 40 }}>
             ...
             {isRH && (
                <div style={{ display: 'flex', gap: 16 }}>
                   <button onClick={handleExport} style={{ padding: '18px 24px', background: 'white', color: '#001D3D', borderRadius: 20, border: '1.5px solid #E2E8F0', fontWeight: 900, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
                      <Download size={18} style={{ color: '#001D3D' }} /> Exporter Excel
                   </button>
             ...
             <div className="stag-glass" style={{ position: 'relative', width: '100%', maxWidth: 740, padding: 0, borderRadius: 40, overflow: 'hidden', border: 'none' }}>
                ...
                <form onSubmit={handleSubmit} style={{ padding: 60, maxHeight: '70vh', overflowY: 'auto' }}>
                   <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                      ...
                      <div className="stag-form-group" style={{ gridColumn: 'span 2' }}>
                         <label className="stag-label">Maître de Stage / Tuteur</label>
                         <select className="stag-input" value={formData.tuteur_id} onChange={e => setFormData({...formData, tuteur_id: e.target.value})}>
                            <option value="">Sélectionnez un tuteur...</option>
                            {tuteurs.map(t => <option key={t.id} value={t.id}>{t.prenom} {t.nom}</option>)}
                         </select>
                      </div>
                      <div className="stag-form-group">

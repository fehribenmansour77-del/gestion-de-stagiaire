...
router.post('/send-otp', otpLimiter, async (req, res, next) => {
    try {
        ...
        // Vérifier si l'utilisateur existe déjà avant d'envoyer le code
...
router.post('/verify-otp', async (req, res, next) => {
    try {
        ...
        const verified = await otpService.verifyOtp(email, code);
...
router.post('/register', async (req, res, next) => {
    try {
        ...
        if (!nom || !prenom || !email || !password || !verificationToken) {
            return res.status(400).json({
                error: 'Tous les champs sont requis (y compris le jeton de vérification)'
            });
        }
        ...
        try {
            ...
            if (!decoded.verified || decoded.email !== email.toLowerCase()) {
                throw new Error('Jeton de vérification invalide pour cet email.');
            }
        } catch (err) {
            return res.status(401).json({
                error: 'Vérification de l\'email expirée ou invalide. Veuillez recommencer.'
            });
        }

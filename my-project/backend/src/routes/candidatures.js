...
router.post('/', upload.fields([
  ...
  try {
    ...
    // Calculer le score de matching IA
    const MatchingService = require('../services/MatchingService');
    const scoreMatching = MatchingService.calculateScore({
      theme,
      filiere,
      niveau,
...
// POST /api/candidatures/re-match - Recalculer tous les scores de matching (Admin)
  ...
  async (req, res, next) => {
    try {
      ...
      for (const cand of candidatures) {
        await MatchingService.updateCandidatureScore(cand);
        count++;
      }

      res.json({
        success: true,
        message: `${count} candidatures mises à jour avec le score IA`,
        count
      });
    } catch (error) {
      next(error);

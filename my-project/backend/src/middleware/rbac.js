...
const PERMISSIONS = {
  // Super Admin - Accès complet de haut niveau
  [ROLES.SUPER_ADMIN]: [
    // Users
    'users:read', 'users:create', 'users:update', 'users:delete',
    // Departments
    'departements:read', 'departements:create', 'departements:update',
    // Stagiaires
...
function requireAccess(resourceType) {
  return (req, res, next) => {
    ...
    // Admin RH & Super Admin ont accès quasi-complet
    if (userRole === ROLES.ADMIN_RH || userRole === ROLES.SUPER_ADMIN) {
      return next();
    }
    ...
    switch (resourceType) {
      case 'stagiaire':
        // Admin RH & Super Admin ont accès
        if (userRole === ROLES.ADMIN_RH || userRole === ROLES.SUPER_ADMIN) {
          return next();
        }
      ...
      case 'presence':
        // Accès selon le rôle
        if ([ROLES.SUPER_ADMIN, ROLES.ADMIN_RH, ROLES.TUTEUR].includes(userRole)) {
          return next();
        }
        break;

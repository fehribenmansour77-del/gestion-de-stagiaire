...
async function requestPasswordReset(email, ipAddress) {
  ...
  try {
    await emailService.sendPasswordResetEmail(normalizedEmail, resetData.token);
  } catch (emailError) {
    console.error('Erreur lors de l\'envoi de l\'email de reset:', emailError);
    // On ne bloque pas le retour car le token est créé, 
    // mais on pourrait loguer l'échec.
  }
